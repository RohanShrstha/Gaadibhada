...
function validate()
{
    if(validateInputs())
    {
        alert("Your post has been submitted, please wait for verification!!");
        form.submit();
    }
    else{
...
function validateInputs()
{
    ...
    {
        setError(title,'Title cannot be more than 30 characters');
        title.focus();
        errorCount ++;
        return false;
    }
    ...
    {
        setError(makeyear,'Provide valid year 20--');
        makeyear.focus();
        errorCount++;
        return false;
    }
    ...
    c=0;
    if(!isEmpty(billbookdV))
    {
        c += fileCheck(billbookd);
        errorCount += c;
    }
    if(!isEmpty(insurancedV))
    {
